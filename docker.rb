    ...
    module Agent
        class Docker<RPC::Agent
            ...
            action "events" do
                ...
                options = {}
                options[:since] = requests[:since] if request[:since]
                options[:until] = requests[:until] if request[:until]
#               [:since, :until].each {|o|
#                       options[o] = request[o] if request[o]
#               }
                begin
                        reply[:events] = _request(:get, "events?since=", options)
                rescue => e
                        reply.fail! "Error querying docker api (GET events/json), #{e}"
                        logger.error e
                end
